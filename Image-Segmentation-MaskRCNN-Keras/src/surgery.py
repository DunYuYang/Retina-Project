...
COCO_WEIGHTS_PATH = os.path.join(ROOT_DIR, "data/mask_rcnn_coco.h5")

# Directory to save logs and model checkpoints, if not provided
# through the command line argument --logs
DEFAULT_LOGS_DIR = os.path.join(ROOT_DIR, "data")

# Path to predefined classes file
PREDEFINED_CLASSES_PATH = os.path.join(ROOT_DIR, "data/predefined_classes.txt")
...
class SurgeryDataset(utils.Dataset):
    def load_VIA(self, dataset_dir, subset, hc=False):
        ...
        #self.add_class("mask", 1, "heteroloba")
        #self.add_class("mask", 2, "japoncius")
        #self.add_class("mask", 3, "punctifer")
        for i in range(len_labels):
            self.add_class("mask", i+1, labelList[i+1])
        
        if hc is True:
            for i in range(1,14):
                self.add_class("mask", i, "{}".format(i))
            self.add_class("mask", 14, "arm")
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        #"name" is the attributes name decided when labeling, etc. 'region_attributes': {name:'a'}
            #if p['name'] == 'heteroloba':
                #class_ids[i] = 1
            #elif p['name'] == 'japoncius':
                #class_ids[i] = 2
            #elif p['name'] == 'punctifer':
                #class_ids[i] = 3
            for k in range(len_labels):
                if p['name'] == labelList[k+1]:
                    class_ids[i] = k+1
...
def detect_and_color_splash(model, image_path=None, video_path=None, out_dir=''):
    ...
    #class_names = ['BG', 'heteroloba', 'japoncius', 'punctifer']
    class_names = ['BG']
    
    for i in range(len_labels):
        class_names.append(labelList[i+1])
...
def loadPredefinedClasses(predefClassesFile):
    if os.path.exists(predefClassesFile) is True:
        ...
        print('No data/predefined_classes.txt')
        return False
...
if __name__ == '__main__':
    ...
    res = loadPredefinedClasses(PREDEFINED_CLASSES_PATH)
    
    if res == False:    # no predefined_classes.txt
        sys.exit()
    
    len_labels = len(labelList) - 1     # eliminate background class
    print('Labels: ' + str(labelList))
    print('Length of labels = ' + str(len_labels))

    # Configurations
    if args.command == "train":
        class TrainConfig(SurgeryConfig):
            NUM_CLASSES = 1 + len_labels
        config = TrainConfig()
    else:
